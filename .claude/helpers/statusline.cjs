...
function getGitInfo() {
  ...
  if (process.platform === 'win32') {
    ...
    const porcelain = safeExec('git status --porcelain', 3000);
    if (porcelain) {
    ...
    const upstream = safeExec('git rev-list --left-right --count HEAD...@{upstream}', 3000);
    if (upstream) {
      const ab = upstream.split(/\s+/);
      result.ahead = parseInt(ab[0], 10) || 0;
      result.behind = parseInt(ab[1], 10) || 0;
    }
...
function getSwarmStatus() {
  ...
  const legacySwarmState = readJSON(path.join(CWD, '.swarm', 'state.json'));
  const configuredMaxAgents =
    getSettings()?.claudeFlow?.swarm?.maxAgents ||
    legacySwarmState?.maxAgents ||
    CONFIG.maxAgents;
  const currentSwarm =
    swarmState?.swarms ? Object.values(swarmState.swarms)[0] : swarmState;

  if (currentSwarm) {
    const updatedAt = currentSwarm.updatedAt || currentSwarm.startedAt || legacySwarmState?.initializedAt;
    const age = updatedAt ? now - new Date(updatedAt).getTime() : Infinity;
    if (age < staleThresholdMs) {
      return {
        activeAgents: (currentSwarm.agents && currentSwarm.agents.length) || currentSwarm.agentCount || 0,
        maxAgents: currentSwarm.maxAgents || configuredMaxAgents,
        coordinationActive: currentSwarm.status === 'running' || ((currentSwarm.agents && currentSwarm.agents.length) || 0) > 0,
      };
    }
  }
  ...
  return { activeAgents: 0, maxAgents: configuredMaxAgents, coordinationActive: false };
}
