...
const SignupConfirmation = ({ currentLanguage }: SignupConfirmationProps) => {
  ...
  // Handle navigation - check for pending action first, then go to appropriate home
  const handleLetsStart = () => {
    const pendingActionStr = localStorage.getItem('pendingAction');
    if (pendingActionStr) {
      try {
        const pendingAction = JSON.parse(pendingActionStr);
        // Keep pending action in storage - login/signup flow will consume it
        if (pendingAction.returnPath) {
          ...
          navigate(`/app/buyer/vendor/${pendingAction.data?.vendorId || ''}`);
          return;
        }
      } catch (e) {
