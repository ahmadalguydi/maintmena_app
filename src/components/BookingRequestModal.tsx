...
const BookingRequestModal = ({
  ...
  const [hasEditedTiming, setHasEditedTiming] = useState(false);
  ...
  const buildDefaultFormData = () => {
    const suggestedSchedule = getSuggestedScheduleDefaults();
    const storedLocation =
      typeof window !== 'undefined'
        ? getStoredServiceLocation(localStorage.getItem('maintmena_last_location'))
        : null;
    const autoCategory = getAutoSelectedVendorCategory(vendorCategories);

    return {
      request_type: 'booking',
      service_category: autoCategory,
      proposed_start_date: suggestedSchedule.bookingDateValue,
      preferred_time_slot: suggestedSchedule.timeSlot,
      location_address: storedLocation?.address || '',
      location_city: storedLocation?.city || '',
      location_country: 'saudi_arabia',
      job_description: '',
      budget_range: inferBudgetRangeFromVendorServices(vendorServices, autoCategory),
      urgency: suggestedSchedule.bookingUrgency,
      requires_deposit: false,
    };
  };
  ...
  const handleCategoryChange = (value: string) => {
    setFormData((prev) => ({
      ...
      budget_range: hasEditedBudget
        ? prev.budget_range
        : inferBudgetRangeFromVendorServices(vendorServices, value) || prev.budget_range,
    }));
  };

  const handleDateChange = (value: string) => {
    const selectedDate = new Date(`${value}T00:00:00`);
    const suggestion = getSuggestedTimeForDate(selectedDate);

    setFormData((prev) => ({
      ...prev,
      proposed_start_date: value,
      preferred_time_slot: hasEditedTiming ? prev.preferred_time_slot : suggestion.timeSlot,
      urgency: hasEditedTiming ? prev.urgency : suggestion.bookingUrgency,
    }));
  };
