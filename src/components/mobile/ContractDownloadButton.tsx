...
export function ContractDownloadButton({
  ...
  const handleDownloadHtml = () => {
    ...
    const blob = new Blob([htmlSnapshot], { type: 'text/html' });
  ...
  const handleDownloadPdf = () => {
    if (pdfUrl) {
      ...
      // For now, download HTML as fallback
      handleDownloadHtml();
  ...
  const handleShareWhatsApp = () => {
    const contractUrl = `${window.location.origin}/app/contract/${contractId}`;
    const message = currentLanguage === 'ar' 
      ? `عقد الخدمة من MaintMENA: ${contractUrl}`
      : `Service Contract from MaintMENA: ${contractUrl}`;
