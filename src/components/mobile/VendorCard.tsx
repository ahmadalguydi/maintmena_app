...
export const VendorCard = ({
    ...
    const totalReviewCount = vendor.reviewCount ?? vendor.total_reviews ?? vendor.reviews?.length ?? 0;
        ...
        <SoftCard animate className="cursor-pointer hover:shadow-lg transition-shadow">
            <div className="space-y-4">
                ...
                <div onClick={() => onNavigate(`/app/buyer/vendor/${vendor.id}`)}>
                    ...
                    <div className="flex items-center gap-4 text-sm flex-wrap">
                        {vendor.seller_rating > 0 && (
                            <div className="flex items-center gap-1">
                                ...
                                <Caption className="text-muted-foreground">({totalReviewCount})</Caption>
                            </div>
                        )}
                    ...
                    {(() => {
                        const jobs = vendor.completed_projects ?? 0;
                        const level = getSellerLevel(jobs);
                        return (
                            <div className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-full bg-primary/8 border border-primary/15">
                                <span className="text-sm leading-none select-none" aria-hidden="true">{level.badge}</span>
                                <Caption className="font-semibold text-primary">
                                    {currentLanguage === 'ar' ? level.labelAr : level.label}
                                </Caption>
                            </div>
                        );
                    })()}
