...
import { X, DollarSign, CheckCircle, Search, AlertCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { SoftCard } from './SoftCard';
import { Heading3, Body, BodySmall, Caption, Label } from './Typography';

export type PricingType = 'fixed' | 'range' | 'inspection';

interface AcceptJobSheetProps {
...
const content = {
    ar: {
        ...
        sar: 'ريال',
        acceptJob: 'قبول الطلب',
        cancel: 'إلغاء',
        bindingNote: 'هذا السعر ملزم ولا يمكن تغييره',
        rangeNote: 'السعر النهائي سيكون ضمن هذا النطاق',
        inspectionNote: 'ستحدد السعر بعد المعاينة',
    },
    en: {
        title: 'Accept Job',
        subtitle: 'Choose pricing method',
        ...
        cancel: 'Cancel',
        bindingNote: 'This price is binding and cannot be changed',
        rangeNote: 'Final price will be within this range',
        inspectionNote: 'You will set the price after inspection',
    }
};

export const AcceptJobSheet = ({
    currentLanguage,
    isOpen,
    onClose,
    onAccept,
    ...
    const handleAccept = () => {
        ...
        if (pricingType === 'fixed') {
            pricing.fixedPrice = Number(fixedPrice);
            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className={cn(
                            "w-full bg-background rounded-t-3xl max-h-[90vh] overflow-y-auto",
                            "pb-safe"
                        )}
                        dir={isRTL ? 'rtl' : 'ltr'}
                    >
                        {/* Header */}
                        <div className="sticky top-0 bg-background px-4 pt-4 pb-2 border-b border-border/30">
                        ...
                        <div className="p-4 space-y-5">
                            {/* Job Summary */}
                            <SoftCard className="p-3 bg-muted/30">
                                <div className={cn("flex items-center gap-3", isRTL && "flex-row-reverse")}>
                                    <span className="text-2xl">{jobDetails.categoryIcon}</span>
                            ...
                            <AnimatePresence mode="wait">
                                {pricingType === 'fixed' && (
                                    ...
                                    >
                                        ...
                                        />
                                        <div className={cn("flex items-center gap-2 text-amber-600", isRTL && "flex-row-reverse")}>
                                            <AlertCircle size={16} />
                                ...
                                {pricingType === 'range' && (
                                    ...
                                    >
                                        ...
                                        <div className={cn("flex items-center gap-2 text-blue-600", isRTL && "flex-row-reverse")}>
                                            <AlertCircle size={16} />
                                ...
                                {pricingType === 'inspection' && (
                                    ...
                                    >
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => setFreeInspection(true)}
                                                className={cn(
                                            ...
                                            >
                                                <BodySmall lang={currentLanguage} className={freeInspection ? "text-primary font-medium" : ""}>
                                                    {t.freeInspection}
                                                </BodySmall>
                                            </button>
                                            <button
                                                onClick={() => setFreeInspection(false)}
                                                className={cn(
                                                    "flex-1 py-3 rounded-xl border-2 transition-all",
                                                    !freeInspection
                                                        ? "border-primary bg-primary/5"
                                        ...
                                        <div className={cn("flex items-center gap-2 text-green-600", isRTL && "flex-row-reverse")}>
                                            <Search size={16} />
