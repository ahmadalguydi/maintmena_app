    ...
    try {
      ...
      for (const file of attachments) {
        ...
        if (uploadError) {
          ...
          toast.error(currentLanguage === 'ar' ? `فشل رفع الملف: ${file.name}` : `Failed to upload file: ${file.name}`);
          continue;
        }
      ...
      const quoteData: any = {
        ...
        estimated_duration: duration ? `${duration} ${parseInt(duration) === 1 ? 'day' : 'days'}` : null,
      ...
      await jobService.submitQuote(quoteData);
      ...
      celebrate({
        ...
        flowTitle: (job as any)?.title || (currentLanguage === 'ar' ? 'تم إرسال العرض' : 'Quote Sent'),
      ...
      // Show specific rate limit error if exists
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-stretch justify-between gap-4 rounded-xl bg-card p-4 shadow-sm border border-[#89440B]/30">
          <div className="flex flex-col gap-1 flex-[2_2_0px]">
            ...
            <p className={`text-foreground text-lg font-medium leading-tight ${currentLanguage === 'ar' ? "font-['Noto_Sans_Arabic']" : ''}`}>{(job as any)?.title || 'Loading...'}</p>
