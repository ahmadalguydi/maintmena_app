-- ============================================================
-- Fix: accept_job_offer now accepts optional p_pricing param
-- The client (useDispatchActions.ts) passes p_pricing when the
-- seller sets their price at acceptance time.  Without this the
-- RPC threw "unexpected named parameter p_pricing".
-- ============================================================
...
BEGIN
    ...
    -- Stamp the maintenance_request so every downstream query
    -- that reads assigned_seller_id will find the job.
    -- Store pricing if provided.
...
-- Keep old 2-param signature for backward compatibility
