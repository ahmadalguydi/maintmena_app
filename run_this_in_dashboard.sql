...
-- This was extracted from the initial migration because the Supabase CLI parser struggles with PL/pgSQL Blocks
...
END IF;
-- We are the winner! Update the session
...
SET offer_status = 'accepted',
    ...
    updated_at = now()
WHERE id = p_offer_id;
-- Auto-close all other offers for this session
UPDATE job_dispatch_offers
SET offer_status = 'auto_closed',
    updated_at = now()
